...
const NAV: { section: string; items: NavItem[] }[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { href: "/users", label: "المستخدمون", icon: "👥", adminOnly: true },
      { href: "/transaction-types", label: "أنواع المعاملات", icon: "🗂️", adminOnly: true },
    ],
  },
];
  ...
  const sidebar = (
    <nav className="flex h-full flex-col overflow-y-auto p-3">
      <div className="mb-4 flex items-center gap-2 px-2 pt-1">
        ...
        <div className="text-[13px] font-bold leading-tight">نظام تدقيق معاملات<br />النقل اللوجستي</div>
      </div>
      {NAV.map((group) => (
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="sticky top-0 z-30 flex h-14 items-center gap-3 border-b border-slate-200 bg-white/90 px-4 backdrop-blur dark:border-slate-800 dark:bg-slate-900/90">
          ...
          <div className="text-left leading-tight">
            ...
            <div className="text-[11px] text-slate-400">{(ROLES as Record<string, string>)[user.role] ?? user.role}</div>
          </div>
          <button onClick={logout} className="btn-secondary !px-2.5 !py-1.5 text-xs">خروج</button>
        </header>
